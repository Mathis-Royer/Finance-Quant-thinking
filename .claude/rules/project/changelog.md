...
| # | Date | Modification |
|---|------|--------------|
| 1 | 2026-02-02 | **Supervised now uses true Sharpe optimization for target weights**: (1) **Replaced softmax with mean-variance optimization**: `compute_rolling_optimal_weights()` now uses `compute_optimal_weights()` which maximizes Sharpe ratio using proper mean-variance framework. (2) **New helper `_get_forward_monthly_returns()`**: Extracts monthly returns from T+1 to T+h (instead of cumulative returns) to enable covariance estimation. (3) **For h≥2**: Uses h monthly observations to compute mean returns and covariance matrix, then optimizes w* = argmax(μ'w / √(w'Σw)). (4) **For h=1**: Falls back to softmax (cannot compute covariance from single observation). (5) **Constraints preserved**: Weights between 5% and 50%, sum to 1. **Impact**: Supervised targets now properly account for factor correlations, not just raw returns. |
| 2 | 2026-02-02 | **Added E2E-P3 as third strategy type for curriculum learning ablation**: (1) **New strategy "E2E-P3"** runs Phase 3 only (Sharpe optimization) without curriculum learning (no Phase 1 binary, no Phase 2 regression). (2) **Updated all pipelines**: `comparison_runner.py`, `three_step_pipeline.py`, `training_strategies.py`. (3) **Updated visualizations**: `holdout_plots.py`, `walk_forward_plots.py`, `constants.py` (P3 abbreviation). (4) **Now 24 base combinations** (3 strategies × 2 allocations × 4 horizons) instead of 16. (5) **Fixed data leakage in `_compute_forward_optimal_weights()`**: Was using window centered around target (including PAST dates), now only uses dates >= target_date. **Purpose**: Compare E2E (3-phase curriculum) vs E2E-P3 (Phase 3 only) to measure curriculum learning benefit. |
| 3 | 2026-02-02 | **Added optimal factor allocation charts to dashboard**: New `render_optimal_allocation_charts()` in `app.py` displays stacked area charts showing what the optimal allocation *would have been* each month based on realized factor returns. (1) New `load_factor_returns()` loads factor data from cache. (2) New `_softmax_weights()` converts returns to allocations with temperature scaling. (3) New `compute_optimal_allocations()` computes weights for holdout period. (4) Shows two charts: Multi (6 factors) and Binary (cyclical vs defensive). Allows comparison with model predictions to assess prediction quality. |
| 3 | 2026-02-02 | **CRITICAL: Fixed return alignment and training bugs**: (1) **Fixed h=1 alignment**: `cumulative_returns` now uses `get_cumulative_factor_returns_for_horizon()` for all horizons (was using raw returns without forward shift for h=1). (2) **Fixed Supervised targets**: Now computes optimal weights on **FORWARD returns** (T+1 to T+horizon) instead of past 24M returns. New functions: `_compute_forward_cumulative_returns()`, `_softmax_weights()`, `_compute_forward_optimal_weights()`. (3) **Equalized epochs**: `epochs_supervised=70` matches E2E total (30+20+20) for fair comparison. (4) **Added ablation option**: `skip_phase1_phase2=True` to test Phase 3-only E2E. |
...
- **Pipeline**: Up to 96 combinations (3 strategies × 2 allocations × 4 horizons × 4 configs)
  - Strategies: **E2E** (3-phase curriculum), **E2E-P3** (Phase 3 only, ablation), **Supervised**
  - Allocations: Binary (2F), Multi (6F)
  - Horizons: 1M, 3M, 6M, 12M
  - **Configs**: baseline, fs (feature selection), hpt (HP tuning), fs+hpt (both)
